...
const ToastDisplay = ({ title, variant }: { title: string; variant?: "default" | "success" | "error" | "warning" | "info" }) => {
...
export const Playground = () => (
  <ToastProvider placement="bottom-right">
    <Stack spacing={2} style={{ padding: '2rem' }}>
      ...
      <Typography variant="body1">Click the buttons below to trigger different types of toast notifications.</Typography>
      <Stack direction="row" spacing={2} style={{ flexWrap: 'wrap', gap: '8px' }}>
...
export const CustomDuration = () => {
  const DurationDemo = () => {
    ...
    return (
      <Button 
        onClick={() => addToast('This stays for 10 seconds.', { duration: 10000, variant: 'warning' })}
      >
        Show 10s Toast
      </Button>
    );
  ...
  return (
    <ToastProvider placement="bottom-center">
      <Stack spacing={2} style={{ padding: '2rem' }}>
        ...
        <Typography variant="body1">You can specify exactly how long the toast should persist on screen.</Typography>
        <div>
          <DurationDemo />
        </div>
      </Stack>
